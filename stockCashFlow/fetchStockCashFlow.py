...
#     readFile(stockId)
    ...
    for name in os.listdir():
        if name.startswith("StockCashFlow_"):
...
def fetch(stockId):
    ...
    url = "https://goodinfo.tw/StockInfo/StockCashFlow.asp?STOCK_ID={}".format(stockId)
    print("fetch url", url)
    headers = {
        "User-Agent" : "Chrome/31.0.1650.63",
        "referer": "https://goodinfo.tw/StockInfo/StockCashFlow.asp?STOCK_ID={}".format(stockId)
    }
...
#         print(row)
    ...
    with open("StockCashFlow_{}.csv".format(stockId), "w", encoding="utf-8", newline="") as f1:
        cw = csv.writer(f1)
